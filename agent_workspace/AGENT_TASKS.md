...
- [x] **Analysis accuracy, Phase 1: traceable, teacher-scorable flags.** Every flag now carries a `flag_key` that survives re-analysis; every analysis run writes a full evidence bundle (`analysis_evidence`) so any flag's measurement, rule and threshold can be recovered; `modal_worker/replay.py` re-decides flags from a bundle offline, with no audio/network/keys; `modal_worker/score_against_annotations.py` computes per-flag-type precision/recall from teacher annotations, with `legacy_matched` and `disagreed` columns so a reader can see how much of a number rests on weaker evidence; CI runs all three worker suites plus the frontend build on every push. `agent_workspace/ACCURACY_BASELINE.md` is published in its unpopulated form — the harvesting query and scoring path are ready to run, but no corpus measurement has been taken (no service-role DB access at authoring time). The gate into Phase 2 is corpus size, not code; see Backlog. 182/182 unit checks, 28/28 coverage behaviours, 45/45 evidence/scoring checks. (2026-08-29)
