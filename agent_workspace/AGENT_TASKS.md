...
- [x] **Analysis accuracy, Plan B: three new detectors — rests, tempo vs marking, wedges.** (1) **Rests survive the score model** instead of being dropped at parse time (`parse_musicxml` keeps `is_rest` entries; `flatten_score_notes`'s existing `if not pitch: continue` guard already excludes them from DTW, so this is safe). `find_wrong_note_candidates` had a latent crash exposed by this — `n.get("pitch", "")` never applies its default because a rest's `pitch` key is present and `None`; fixed to `n.get("pitch") or ""`. (2) **`find_rest_violations` catches playing through a written rest** — gated as strictly as the wrong-note detector (confidence ≥65, cents_spread cap, ≥150ms held, onset must start >150ms past the rest's own start so a decaying note is never mistaken for a new one). `collect_rest_windows` converts `duration_beats` (a quarterLength) through the existing `quarter_lengths_per_beat` helper before comparing to the minimum-beats threshold — a dotted-quarter rest in 6/8 is one beat, not 1.5, the same unit trap `analyze_timing_vs_score` was fixed for earlier. Rest violations are stamped `priority=3`, above the `priority=2` timing-fit findings (placement/drift/duration) they share the `"timing"` type and dedup slot with — "you played through a written rest" is the more fundamental and rarer fault, and is the one that must survive when only one can. (3) **`check_tempo_vs_marking` compares the played tempo to the marked one**, reported as fact not fault (deliberate slow practice is indistinguishable from error here) — silent inside ±15%, silent when the marking is a word ("Allegro") rather than a number. (4) **`analyze_wedges` catches a written crescendo/diminuendo that never happens** — both `parse_musicxml` (`DynamicWedge` spanners) and the vision reader (runs of consecutive `dyn: "cresc"/"dim"` measures) now populate `score["wedges"]`; the check compares the medians of the first and second half of the span's measured per-note loudness rather than fitting a slope, so one loud note can't fake it. 283/283 unit checks (249 before), 36/36 coverage behaviours (30 before), 61/61 evidence/scoring checks. None of the four new thresholds (`_REST_MIN_BEATS`, `_TEMPO_MARK_PCT`, `_WEDGE_MIN_NOTES`/`_WEDGE_MIN_DB`) is corpus-calibrated — see Backlog. (2026-08-29)
